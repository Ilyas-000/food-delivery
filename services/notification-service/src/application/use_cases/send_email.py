"""Send email notification use case."""

import structlog

from src.application.dto.notification import NotificationResponseDTO, SendEmailDTO
...
class SendEmailUseCase:
    ...
    async def execute(self, dto: SendEmailDTO) -> NotificationResponseDTO:
        """Render, send, and persist email notification."""
        ...
        provider_message_id = await self._email_client.send(
        ...
        try:
            await self._event_publisher.publish_email_sent(saved)
        except Exception:
            logger.exception(
                "notification.email_event_publish_failed",
